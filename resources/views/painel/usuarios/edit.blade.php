...
<div class="d-flex align-items-start justify-content-between flex-wrap gap-2">
  <div>
    <h3 class="fw-bold mb-1 text-white">Editar Usuário</h3>
    <div class="text-muted" style="color: rgba(255,255,255,.65) !important;">
      Atualize dados, permissões e vínculo operacional
    </div>
  </div>
...
<div class="container-fluid px-4">
  <div class="dash-card p-3 p-md-4">
    ...
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-2">
      <div>
        ...
        <p class="section-sub">Edite as informações principais, permissões e vínculo operacional.</p>
      </div>
    </div>
    ...
    <form action="{{ route('usuarios.update', $usuario->id) }}" method="POST">
      ...
      <div class="row g-3">
        ...
        <div class="col-md-6">
          ...
          <input type="text" name="name" value="{{ old('name', $usuario->name) }}" class="form-control" placeholder="Nome completo" required>
        </div>

        <div class="col-md-6">
          <label class="form-label">E-mail</label>
          <input type="email" name="email" value="{{ old('email', $usuario->email) }}" class="form-control" placeholder="email@empresa.com" required>
        </div>

        <div class="col-md-4">
          <label class="form-label">CPF</label>
          <input type="text" name="cpf" value="{{ old('cpf', $usuario->cpf) }}" class="form-control" placeholder="000.000.000-00" required>
        ...
        <div class="col-md-4">
          ...
          <small class="help-muted">Preencha apenas se quiser alterar.</small>
        </div>

        <div class="col-md-4">
          <label class="form-label">Ativo</label>
          <select name="ativo" class="form-select" required>
            <option value="1" {{ (string)old('ativo', (int)$usuario->ativo) === '1' ? 'selected' : '' }}>Sim</option>
            <option value="0" {{ (string)old('ativo', (int)$usuario->ativo) === '0' ? 'selected' : '' }}>Não</option>
          </select>
  ...
  @else
    <select name="cliente_id" class="form-select">
      ...
      @foreach(($clientes ?? []) as $c)
        <option value="{{ $c->id }}"
          {{ (string)old('cliente_id', $usuario->cliente_id) === (string)$c->id ? 'selected' : '' }}>
          #{{ $c->id }} — {{ $c->nome }}
        </option>
      @endforeach
    </select>
    <small class="help-muted">Master pode alterar o cliente do usuário.</small>
  @endif
</div>
  ...
  @foreach(($jornadas ?? []) as $j)
    <option value="{{ $j['id'] }}"
      {{ old('jornada_id', $usuario->jornada_id ?? '') == $j['id'] ? 'selected' : '' }}>
      {{ $j['nome'] ?? $j['id'] }}
    </option>
  @endforeach
</select>
...
<p class="section-title mb-1">Férias</p>
<p class="section-sub mb-3">Quando ativo, o sistema sinaliza que o usuário está fora da operação regular.</p>

<div class="row g-3">
  <div class="col-md-4">
    <label class="form-label">Em férias?</label>
    <select name="ferias_ativo" class="form-select" required>
      <option value="0" {{ (string)old('ferias_ativo', $usuario->ferias_ativo ?? 0) === '0' ? 'selected' : '' }}>Não</option>
      <option value="1" {{ (string)old('ferias_ativo', $usuario->ferias_ativo ?? 0) === '1' ? 'selected' : '' }}>Sim</option>
    </select>
  </div>

  <div class="col-md-4">
    <label class="form-label">Início</label>
    <input type="date" name="ferias_inicio" class="form-control"
      value="{{ old('ferias_inicio', $usuario->ferias_inicio ?? '') }}">
  </div>

  <div class="col-md-4">
    <label class="form-label">Fim</label>
    <input type="date" name="ferias_fim" class="form-control"
      value="{{ old('ferias_fim', $usuario->ferias_fim ?? '') }}">
  </div>
</div>
...
<script>
  ...
  const turnoAtual = @json(old('turno_id', $usuario->turno_id ?? ''));
